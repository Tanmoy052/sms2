...
import { LayoutDashboard, GraduationCap, Users, Bell, FolderKanban, LogOut, Menu, X, CalendarCheck } from "lucide-react"
    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b z-50 flex items-center justify-between px-4">
        <div className="flex items-center gap-2">
          <Image src="/images/cgec-logo.png" alt="CGEC Logo" width={32} height={32} className="rounded-full" priority />
      ...
      {mobileOpen && (
        <div className="lg:hidden fixed inset-0 top-14 bg-white z-40 p-4">
          <nav className="space-y-2">
            {navItems.map((item) => (
              <NavItem key={item.href} item={item} isActive={pathname === item.href} onClick={closeMobile} />
            ))}
          </nav>
          <div className="mt-4 pt-4 border-t">
            <Button variant="ghost" className="w-full justify-start gap-3" type="button" onClick={handleLogout}>
